...
struct RLPTests {
		...
		payloadSignatures: [
			Flow.TransactionSignature(
				...
				signature: Flow.Signature(
					hex: "f7225388c1d69d57e6251c9fda50cbbf9e05131e5adb81e5aa0422402f048162"
				).data
			),
		],
		envelopeSignatures: []
	...
	func zeroPayloadSigsKey() {
		guard var sig = baseTx.payloadSignatures.first else {
			Issue.record("Missing base payload signature")
			return
		}

		sig = sig.buildUpon(keyIndex: 0)

		let tx = baseTx.buildUpOn(payloadSignatures: [sig])
