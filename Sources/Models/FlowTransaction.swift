...
public extension Flow {
	...
	struct Transaction: Sendable {
		...
		public func buildUpOn(
			script: Flow.Script? = nil,
			...
			envelopeSignatures: [Flow.TransactionSignature]? = nil
		) -> Transaction {
			Transaction(
		...
		public var encodedEnvelope: Data? {
			RLP.encode(payloadEnvelope.rlpList)
		}
		...
		var payloadEnvelope: PayloadEnvelope {
			let signatures = payloadSignatures
				.map { sig in
					EnvelopeSignature(
						signerIndex: signers[sig.address] ?? -1,
		...
		func getSingerIndex(address: Flow.Address) -> Int? {
			signers.first { $0.key == address }?.value
		}
	}
}
...
extension Flow.Transaction {
	...
	struct PayloadEnvelope: RLPEncodable, Sendable {
		...
		var rlpList: [Any] {
			[payload.rlpList, payloadSignatures.compactMap { sig in [sig.signerIndex, sig.keyIndex, sig.signature] }]
		}
	}
...
public extension Flow {
	...
	struct TransactionResult: Codable, Sendable {
		...
		public var errorCode: FvmErrorCode? {
			...
			return FvmErrorCode.allCases.first { errorMessage.contains($0.errorTag) }
		}
	}
