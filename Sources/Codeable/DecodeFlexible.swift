...
extension KeyedDecodingContainer {
	...
	func decodeFlexible<T>(
	_ types: [Decodable.Type],
	as: T.Type,
	forKey key: Key
	) throws -> T {
		for type in types {
			if let convertibleType = type as? Convertible.Type,
				let value = try? decode(type, forKey: key) as? Convertible,
				let converted = value.convert(to: T.self) {
				_ = convertibleType // keep reference to satisfy compiler, no-op
				return converted
			}
		}
