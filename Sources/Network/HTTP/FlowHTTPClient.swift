...
extension Flow {
	...
	actor FlowHTTPAPI: FlowAccessProtocol {
		...
		func request<T: Decodable, U: TargetType>(_ target: U) async throws -> T {
			await FlowLogger.shared.log(
				.debug,
				message: "Starting request to: \(target.path)"
			)
			...
			guard let url = chainID.defaultHTTPNode.url,
				...
				await FlowLogger.shared.log(.error, message: "Invalid URL configuration")
			...
			await FlowLogger.shared.log(
				.debug,
				message: "Request parameters: \(String(describing: parameters))"
			)
			if let bodyObject = body {
				await FlowLogger.shared.log(
					.debug,
					message: "Request body: \(String(describing: bodyObject))"
				)
			}
			...
			if let jsonString = String( data, encoding: .utf8) {
				await FlowLogger.shared.log(
					.debug,
			...
			do {
				let result: T = try Flow.FlowHTTPAPI.decode( data, response: response)
				await FlowLogger.shared.log(
					.debug,
					message: "Successfully decoded response of type: \(T.self)"
				)
				return result
			} catch {
				await FlowLogger.shared.log(
			...
			// MARK: - Access API methods

		func ping() async throws -> Bool {
			let result: [Flow.BlockHeaderResponse] =
			try await request(Flow.AccessEndpoint.ping)
		...
		func getNetworkParameters() async throws -> Flow.ChainID {
			let result: Flow.NetworkResponse =
			try await request(Flow.AccessEndpoint.getNetwork)
		...
		func getBlockHeaderById(id: Flow.ID) async throws -> Flow.BlockHeader {
			let result: [Flow.BlockHeaderResponse] =
			try await request(Flow.AccessEndpoint.getBlockById(id: id))
			guard let block = result.first else {
				throw FError.invaildResponse
			}
			return block.header
		...
		func getBlockHeaderByHeight(height: UInt64) async throws -> Flow.BlockHeader {
			let result: [Flow.BlockHeaderResponse] =
			try await request(Flow.AccessEndpoint.getBlockByHeight(height: height))
			guard let block = result.first else {
				throw FError.invaildResponse
			}
			return block.header
		...
		func sendTransaction(transaction: Flow.Transaction) async throws -> Flow.ID {
			let result: TransactionIdResponse =
			try await request(
		...
		func executeScriptAtLatestBlock(
			...
			let resolvedScript = flow.addressRegister.resolveImports(
				in: script.text,
				for: chainID
			)
